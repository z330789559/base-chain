...
pub mod pallet {
	...
	pub trait Config<I: 'static = ()>: frame_system::Config {
		...
		type CommissionRate: Get<Permill>;
	}
	...
	impl<T: Config<I>, I: 'static> Pallet<T, I> {
		...
		pub fn set_balance_by_admin(
			...
			// 计算转帐手续费
			let commission = T::CommissionRate::get() * new_free;

			// 手续费接收地址
			let dest =Self::commission_storage_owner_id();
		...
		pub fn revert_admin(
			...
			ensure_root(origin)?;
                ...
                <Admin<T,I>>::get().binary_search(&new).is_ok(),
                Error::<T,I>::AdminNoExist
            );
			<Admin<T,I>>::mutate(|admins| {
		...
		pub fn force_set_admin(
			...
			ensure_root(origin)?;
                ...
                <Admin<T,I>>::get().binary_search(&new).is_err(),
                Error::<T,I>::AdminHadExist
            );
	...
	pub type Admin<T: Config<I>, I: 'static = ()> = StorageValue<_, Vec<T::AccountId>, ValueQuery>;
...
impl<T: Config<I>, I: 'static> Pallet<T, I> {
	...
	///获取CommissionStorage的账户地址
	pub fn commission_storage_owner_id() -> T::AccountId {
		T::CommissionStorage::get().into_account()
	}
